...
;
; VERSION : 1.2  01/93  INITIAL RELEASE
; VERSION : 1.21  7/93  CORRECTED RELAY BOARD TEST CODE
;
...
; VERSION 1.7  11/2022  CONDITIONAL ASSEMBLY TARGETS TEC-1F, SOUTHERN CROSS, HARDWARE AND SOFTWARE KEYBOARDS
; VERSION 1.8  03/2023  ADD 'UPDATE','SCBUG' TO SYSTEM CALLS
;                       ADD A SYSTEM CALL TO RETURN THE BASE ADDRESS OF THE RAM VARIABLES
;                       ADDED SYSTEM CALLS FOR STRINGS AND BIT FIELDS TO RETURN BOARD TYPE AND KEYBOARD TYPE
;                       FIXED THE SERIAL GO ROUTINE SO IT RETURNS TO THE SERIAL MONITOR WHEN RET
;                       REMOVE INCH/PUTCH SERIAL REDIRECTION
;                       ADDED A BIT BANG SERIAL INITIALISATION SYSTEM CALL
;                       TRAPPED UNUSED SYSTEM CALLS
;                       CONVERT TEC-1F KEY CODES FOR KEYS ABOVE 'F' INTO SC KEY CODES
;                       SCANKEY ROUTINE RETURNS SHIFT KEY STATUS WITH KEYPRESS
;                       TURN ON THE SPEAKER AT STARTUP
; VERSION 1.8 
;       ACIA 03/2023    PATCHED ACIA SERIAL CODE
;
...
; 6850 ACIA REGISTERS
;
CONTROL	.EQU	80H	;(WRITE) CONTROL REGISTER
STATUS	.EQU	80H	;(READ) STATUS REGISTER
TDR	.EQU	81H	;(WRITE) TRANSMIT DATA REGISTER
RDR	.EQU	81H	;(READ) RECEIVE DATA REGISTER
;
...
;
	.ORG	VARBLS
;
;BIT BANG SERIAL MONITOR (SCBUG) VARIABLES
;
...
;
	.ORG	0066H
; THE TEC-1F HAS NMI CONNECTED TO THE INVERTED
; 74C923 KEYBOARD ENCODER DA SO JUST RETURN
#IFDEF TEC-1F
    RETI
#ELSE
	PUSH	HL
...
SYSJMP	.DW	MAIN,VERS,DISADD,DISBYT,CLRBUF,SCAND,CONBYT,CONVHI
	...
	.DW RAND,INDEXB,INDEXW,MUSIC,TONE,BEEP,SKATE,AciaTx
	.DW AciaRx,ASCHEX,WWATCH,RWATCH,ONESEC,RLSTEP,DELONE,SCANKEY
	
	.DW INTELH,SPLIT,SNDMSG,BITASC,WRDASC,BYTASC,NYBASC,PCBTYP
	.DW PRNTSZ,KBDTYP,UPDATE,VARRAM,AciaIni,SCBUG,TRAP,TRAP
...
;
	...
	LD	HL,SSTEP
	LD	(RST38),HL	;SINGLE STEPPER INTERRUPT
	LD	HL,STOP
...
;	LD	HL,B4800	;DEFAULT BAUD RATE
	CALL AciaIni	;INITIALISE THE ACIA SERIAL PORT
;
	LD	HL,0100H
...
INTEL1	XOR	A
	...
	CALL	AciaRx	;WAIT FOR THE RECORD MARK
...
LF	.EQU	0AH
;
SCBUG	LD	HL,B4800	;INITIALISE THE BAUD RATE
...
;
START2	LD	A,'>'
	CALL	AciaTx	;DISPLAY THE PROMPT
START3	CALL	AciaRx	;GET A CHARACTER FROM THE CONSOLE
...
START4	CP	(HL)	;IN THE LIST?
	...
	JR	START3
START5	LD	A,C	;GET ORIGINAL CHAR BACK
	CALL	AciaTx	;TO ECHO IT
...
;
	...
	JR	Z,MDIFY2
	LD	A,'?'
	CALL	AciaTx	;NOT THE SAME DATA, PROBABLY NO RAM THERE
;
...
;      NEXT INSTRUCTION
;
PRNTSZ	POP	HL	;RETURN ADDRESS IS START OF STRING
LOOPSZ	LD	A,(HL)	;GET CHARACTER
	CALL	AciaTx	;AND SEND IT
	INC	HL	;POINT TO THE NEXT
	OR	A	;CHARACTER AND
	JR	NZ,LOOPSZ	;RETURN IF CHAR = 0
	JP	(HL)	;RETURN TO LOCATION AFTER 0 TERMINATOR
;-------------------------------------------
; MC6850 ASYNCHRONOUS COMMUNICATIONS ADAPTER
;-------------------------------------------
;
;CLOCK DIVISOR
;
...
;initialise ACIA  8 bit word, No parity, 2 stop, divide by 64 for 115200 baud (default)
...
;--------------------------
AciaTx	ld	b,a	;save the character  for later
AciaTx1	in	a,(STATUS)	;get the ACIA status 
	bit	TDRE,a	;is the TDRE bit high?
	jr	z,AciaTx1	;no, the TDR is not empty
...
;---------------------------------
AciaRx	in	a,(STATUS)	;get the ACIA status
	bit	RDRF,a	;is the RDRF bit high?
	jr	z,AciaRx	;no, the RDR is empty
